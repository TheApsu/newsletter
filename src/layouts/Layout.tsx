...
export default function Layout({ uid }: SignInPageProps) {
  ...
  const activeLink = (isActive: boolean) =>
    isActive
      ? 'bg-primary text-white font-bold transition-all  block p-3 rounded-lg [&_.wrapper-svg]:bg-white [&_svg]:text-primary shadow-md'
  ...
  return (
    <div style={{ height: '100vh' }} className='bg-gray-200'>
      ...
      >
        <Pane
          minSize={50}
          className='bg-white shadow-menuShadow transition-all'
        >
          {!mQuery.matches && (
            <LayoutHeader
              type='close'
